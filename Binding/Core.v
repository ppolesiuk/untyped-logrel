...
Section Arrows.
  ...
  Definition arrow {Arr} {ArrEq : EqCore2 Arr} {AC : ArrowCore Arr} := Arr.

  Notation " 'ı' " := (arrow_id _) : bind_scope.
  Notation " f ∘ g " := (arrow_comp f%bind g%bind) (at level 40, left associativity) : bind_scope.

  Class Arrow (Arr : Obj → Obj → Type) {ArrEq : EqCore2 Arr} {AC : ArrowCore Arr} : Prop :=
    { arrow_comp_id_l {A B} (f : Arr A B) : ı ∘ f ≡ f;
      ...
      arrow_comp_assoc {A B C D} (f : Arr C D) (g : Arr B C) (h : Arr A B) :
        f ∘ g ∘ h ≡ f ∘ (g ∘ h);
      arrow_comp_proper
        A B C :: Proper (equal ==> equal ==> equal) (arrow_comp (A := A) (B := B) (C := C))}.

  Context {Arr : Obj → Obj → Type} {ArrEq : EqCore2 Arr} {AC : ArrowCore Arr}.
  ...
  (* Functor parameterised w/ equality relation for the resulting type *)
  Class FunctorEq (F : Obj → Type) {FC : FunctorCore F} {EF : EqCore1 F} : Prop :=
    { fmap_ı {A} (t : F A) : fmap ı t ≡ t;
      fmap_comp {A B C} (f : Arr B C) (g : Arr A B) t :
        fmap f (fmap g t) ≡ fmap (f ∘ g) t;
      fmap_ext A B ::
        Proper (equal ==> equal ==> equal) (fmap (F := F) (A := A) (B := B))
    }.
...
Section Substitutions.
  ...
  Class Subst
        (Arr : Obj → Obj → Type) {ArrEq : EqCore2 Arr} {AC : ArrowCore Arr}
        (Sub : Obj → Obj → Type) {SubEq : EqCore2 Sub} {SC : ArrowCore Sub}
        {ASC : SubstCore Arr Sub}  : Prop :=
    { arrow_subst_id {A} : ı ̂ ≡ (ı : Sub A A);
      arrow_subst_comp {A B C} (f : Arr B C) (g : Arr A B) :
        (f ∘ g)̂ ≡ f ̂ ∘ g ̂;
      arrow_subst_proper A B :: Proper (equal ==> equal) (subst_of_arr (A := A) (B := B))}.

    (** ** Binding *)

    Context {Arr : Obj → Obj → Type} {EqArr : EqCore2 Arr} {AC : ArrowCore Arr}.
    Context {Sub : Obj → Obj → Type} {EqSub : EqCore2 Sub} {SC : ArrowCore Sub}.
    ...
    Class BindMapPureEq (F : Obj → Type) {MapF : FunctorCore F} {BindF : BindCore F} {EF : EqCore1 F} : Prop :=
      { bind_map_eq {A B} (f : Arr A B) g t
                 (EQ : f ̂ ≡ g) :
          fmap f t ≡ bind g t
      }.

    Class BindMapCommEq (F : Obj → Type) {MapF : FunctorCore F} {BindF : BindCore F} {EF : EqCore1 F} : Prop :=
      { bind_map_comm_eq {A B₁ B₂ C} (f₁ : Arr B₁ C) (f₂ : Arr A B₂) (g₁ : Sub A B₁) g₂ t
                 (EQ : g₂ ∘ f₂ ̂ ≡ f₁ ̂ ∘ g₁) :
          bind g₂ (fmap f₂ t) ≡ fmap f₁ (bind g₁ t)
      }.

    Class BindEq (F : Obj → Type) {BindF : BindCore F} {EF : EqCore1 F} : Prop :=
      { bind_ı {A} (f : Sub A A) (t : F A) (EQ : f ≡ ı) :
          bind f t ≡ t;
        bind_comp {A B C} (f : Sub B C) (g : Sub A B) h (t : F A)
                       (EQ : f ∘ g ≡ h) :
          bind f (bind g t) ≡ bind h t;
        bind_ext {A B} :: Proper (equal ==> equal ==> equal) (bind (A := A) (B := B))
      }.
...
Module SubNotations.
  ...
  (* This is a bit of a hack, but using the same "Arrow" class as a
     definition of what is in essence a category makes type inference
     go a bit wonky sometimes, and confuse renamings and
     substitutions. *)
...
Section Lifting.
  ...
  (* Sometimes wonder if adding G to the notation explicitly wouldn't
     help readability... *)
  Notation " f ↑ " := (lift f%bind) (at level 30) : bind_scope.

  Class Liftable {Arr} {EqArr : EqCore2 Arr} {AC : ArrowCore Arr}
        (G : Obj → Obj) {LC : LiftableCore Arr G} : Prop :=
    { lift_id {A} (f : Arr A A) (EQ : f ≡ ı) : f ↑ ≡ ı;
      lift_comp {A B C} (f : Arr B C) (g : Arr A B) h (EQ : f ∘ g ≡ h) :
        f ↑ ∘ g ↑ ≡ h ↑ ;
      lift_proper A B :: Proper (equal ==> equal) (lift (A := A) (B := B))
    }.
